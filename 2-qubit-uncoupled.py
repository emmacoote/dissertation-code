"""
Two-Qubit Uncoupled Hybrid Engine

Extends the single-qubit hybrid engine to a two-qubit system with a
two-qubit catalyst. The system qubits are uncoupled (no ZZ interaction)
and evolve independently under sequential XY coupling with the catalyst.
Measurement is restricted to qubit A, with feedback applied to both
qubits sequentially.

Reproduces the uncoupled two-qubit hybrid engine results of the
dissertation, including work extraction, efficiency, COP, entropy
production, entanglement, and coherence diagnostics.
...
# XY coupling Hamiltonian builder
...
# Weak measurement operators on qubit A only - enables non-trivial feedback on qubit B via quantum back-action
